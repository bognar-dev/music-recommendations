...
export default function StepThreeForm({ recommendations }: StepThreeFormProps) {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name.startsWith('songRatings.')) {
            ...
            const existingSongRating = surveyData.stepSeven.songRatings.find(
            ...
            const updatedRating = {
                ...
                modelId: surveyData.stepSeven.modelId,
                rating: Number(value)
            };

            const updatedSongRatings = existingSongRating
                ? surveyData.stepSeven.songRatings.map(sr =>
                    sr.songId === Number(songId) ? updatedRating : sr
                )
                : [...surveyData.stepSeven.songRatings, updatedRating];

            updateSurveyDetails('stepSeven', {
        ...
        if (name.startsWith('modelRating.')) {
            ...
            updateSurveyDetails('stepSeven', {
                modelRating: {
                    ...surveyData.stepSeven.modelRating,
                    [field]: Number(value)
                }
            });
        }
                                ...
                                <li key={track.id} className="flex items-center space-x-4 border-b pb-4 last:border-b-0">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <ul className="flex space-x-2">
                                            <VinylRating name={`songRatings.${track.id}.rating`} value={surveyData.stepSeven.songRatings.find(sr => sr.songId === track.id)?.rating || 0} onChange={handleInputChange} />
                                        </ul>
                                    </div>
                                </li>
                            ))}
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <ul className="flex space-x-2 mt-2">
                                    <VinylRating name="modelRating.relevance" value={surveyData.stepSeven.modelRating.relevance} onChange={handleInputChange} />
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <ul className="flex space-x-2 mt-2">
                                    <VinylRating name="modelRating.novelty" value={surveyData.stepSeven.modelRating.novelty} onChange={handleInputChange} />
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <ul className="flex space-x-2 mt-2">
                                    <VinylRating name="modelRating.satisfaction" value={surveyData.stepSeven.modelRating.satisfaction} onChange={handleInputChange} />
                                </ul>
                            </div>
                        </div>
                    </CardContent>
