...
export default function StepThreeForm({ recommendations }: StepThreeFormProps) {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name.startsWith('songRatings.')) {
            ...
            const existingSongRating = surveyData.stepEight.songRatings.find(
            ...
            const updatedRating = {
                ...
                modelId: surveyData.stepEight.modelId,
                rating: Number(value)
            };

            const updatedSongRatings = existingSongRating
                ? surveyData.stepEight.songRatings.map(sr =>
                    sr.songId === Number(songId) ? updatedRating : sr
                )
                : [...surveyData.stepEight.songRatings, updatedRating];

            updateSurveyDetails('stepEight', {
        ...
        if (name.startsWith('modelRating.')) {
            ...
            updateSurveyDetails('stepEight', {
                modelRating: {
                    ...surveyData.stepEight.modelRating,
                    [field]: Number(value)
                }
            });
        }
