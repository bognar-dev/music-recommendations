...
export default function StepTwoForm({ recommendations }: StepTwoFormProps) {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (name.startsWith('songRatings.')) {
            ...
            const existingSongRating = surveyData.stepFive.songRatings.find(
            ...
            const updatedRating = {
                ...
                modelId: surveyData.stepFive.modelId,
                rating: Number(value)
            };

            const updatedSongRatings = existingSongRating
                ? surveyData.stepFive.songRatings.map(sr =>
                    sr.songId === Number(songId) ? updatedRating : sr
                )
                : [...surveyData.stepFive.songRatings, updatedRating];

            updateSurveyDetails('stepFive', {
        ...
        if (name.startsWith('modelRating.')) {
            ...
            updateSurveyDetails('stepFive', {
                modelRating: {
                    ...surveyData.stepFive.modelRating,
                    [field]: Number(value)
                }
            });
        }
                                ...
                                <li key={track.id} className="flex items-center space-x-4 border-b pb-4 last:border-b-0">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <ul className="flex space-x-2">
                                            <VinylRating name={`songRatings.${track.id}.rating`} value={surveyData.stepFive.songRatings.find(sr => sr.songId === track.id)?.rating || 0} onChange={handleInputChange} />
                                        </ul>
                                    </div>
                                </li>
                            ))}
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <ul className="flex space-x-2 mt-2">
                                    <VinylRating name="modelRating.relevance" value={surveyData.stepFive.modelRating.relevance} onChange={handleInputChange} />
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <ul className="flex space-x-2 mt-2">
                                    <VinylRating name="modelRating.novelty" value={surveyData.stepFive.modelRating.novelty} onChange={handleInputChange} />
                                </ul>
                            </div>
                            ...
                            <div>
                                ...
                                <ul className="flex space-x-2 mt-2">
                                    <VinylRating name="modelRating.satisfaction" value={surveyData.stepFive.modelRating.satisfaction} onChange={handleInputChange} />
                                </ul>
                            </div>
                        </div>
                    </CardContent>
